...
class BinarySearchTree {
 public:
  ...
  bool contains(const Type& to_find) const { return contains(to_find, m_root); }
  ...
  void insert(const Type& to_insert) { insert(to_insert, m_root); }
  void remove(const Type& to_remove) { remove(to_remove, m_root); }
 ...
 private:
  ...
  bool contains(const Type& to_find, Node* root) const {
    // Again, Operator < and Operator == Is Assumed Here - Works With Primitive
    // Types
    if (!root) {
      return false;
  ...
  void remove(const Type& to_remove, Node*& root) {
    ...
    if (root->m_left && root->m_right) {
      // Two Children - Idea is to get inorder pre/post successor instead and go
      // delete that until we get to 1 child or none
      Node* next_to_delete = findMax(root->m_left);
      root->m_data = next_to_delete->m_data;
      // Could have returned a pair of next_to_delete and parent of
      // And Delete Directly
      // Traverse Same n Times again  like findMax (Has to be better)
      remove(next_to_delete->m_data, root->m_left);
    } else {
      // One Or Zero
      Node* temp =
          root;  // Will Delete after modifying Where we Should Connect to
